...
object ChatMessage {
  final case class IncomingChatMessage(to: User.Id, payload: String)
    ...
    override def toString: String =
      s"IncomingChatMessage(to=${to.toString}, payload is hidden)"
  }
  final case class GeneralChatMessage(
    ...
    override def toString: String =
      s"GeneralChatMessage(from=${from.toString}, to=${to.toString}, unix time=${dateTimeUtcEpochSeconds.toString}, payload is hidden)"
  }
}
...
final case class SignUpOrInRequest(id: String, password: String, email: String)
  ...
  def validate: Either[InvalidRequest, ValidSignUpOrInRequest] =
    ...
    ).mapN((id, password, email) =>
        ValidSignUpOrInRequest(id, password, email)
      )
...
object User {
  ...
  object Password {
    ...
    private def validate2SameAdjacentCharacters(
      ...
      if (
        password.length >= 2 && password.zip(password.tail).exists {
          case (a, b) => a == b
        }
      )
