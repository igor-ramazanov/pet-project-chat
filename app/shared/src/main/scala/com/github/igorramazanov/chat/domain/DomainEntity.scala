...
object ChatMessage {
  final case class IncomingChatMessage(to: User.Id, payload: String) extends ChatMessage {
  ...
  final case class GeneralChatMessage(
      from: User.Id,
      to: User.Id,
      payload: String,
...
final case class SignUpOrInRequest(id: String, password: String, email: String)
  ...
  def validate: Either[InvalidRequest, ValidSignUpOrInRequest] =
    (User.Id.validate(id), User.Password.validate(password), User.Email.validate(email))
      .mapN((id, password, email) => ValidSignUpOrInRequest(id, password, email))
      .leftMap(validationErrors => InvalidRequest(validationErrors.map(_.errorMessage)))
      .toEither
...
final case class InvalidRequest(validationErrors: NonEmptyChain[String]) extends DomainEntity

final case class ValidSignUpOrInRequest private (
    id: User.Id,
...
object ValidSignUpOrInRequest {
  ...
  def unsafeCreate(id: String, password: String, email: String): ValidSignUpOrInRequest =
    ValidSignUpOrInRequest(User.Id(id), User.Password(password), User.Email(email))

  def fromUser(user: User): ValidSignUpOrInRequest =
    ValidSignUpOrInRequest(user.id, user.password, user.email)
}

final case class User private (id: User.Id, password: User.Password, email: User.Email)
    extends DomainEntity {
  override def toString: String =
...
object User {
  ...
  object Id {
    ...
    private def validateContainsOnlyLowercaseLatinCharacters(id: String): ValidationResult[String] =
  ...
  object Password {
    ...
    private def validateLengthLonger(password: String): ValidationResult[String] =
      if (password.length > 128)
        PasswordValidationError.LongerThan128Characters.invalidNec
      else password.validNec

    private def validate2SameAdjacentCharacters(password: String): ValidationResult[String] =
    ...
    private def validateContainsLowercaseCharacter(password: String): ValidationResult[String] =
      if (!password.exists(lowercase))
        PasswordValidationError.DoesNotContainLowercaseCharacter.invalidNec
      else password.validNec

    private def validateContainsUppercaseCharacter(password: String): ValidationResult[String] =
      if (!password.exists(uppercase))
        PasswordValidationError.DoesNotContainUppercaseCharacter.invalidNec
      else password.validNec

    private def validateContainsSpecialCharacter(password: String): ValidationResult[String] =
      if (!password.exists(special))
        PasswordValidationError.DoesNotContainSpecialCharacter.invalidNec
      else password.validNec

    private def validateContainsDigit(password: String): ValidationResult[String] =
  ...
  object Email {
    ...
    private def `validateAtLeast1@Character`(email: String): ValidationResult[String] =
  ...
  def safeCreate(
      ...
      password: String,
      email: String
  ): ValidatedNec[DomainEntityValidationError, User] =
  ...
  object Implicits {
    ...
    implicit val passwordEq: Eq[Password] = (x: Password, y: Password) => x.value === y.value
    implicit val emailEq: Eq[Email]       = (x: Email, y: Email) => x.value === y.value
