...
val jvmSettings = Seq(
  ...
  Compile / scalacOptions := ("-Xplugin:" + (baseDirectory
    .in(root)
    .value / ("paradise_" + scalaVersion.value + "-2.1.1.jar")).absolutePath) +: compilerOptions,
...
val jsSettings = Seq(
  ...
  (Compile / scalacOptions) ++= compilerOptions.filterNot(
    Set("-Ywarn-unused:params", "-Ywarn-value-discard").apply
  ),
  (Compile / console / scalacOptions) --= Seq("-Ywarn-unused:imports", "-Xfatal-warnings")
)
...
def addFrontendResourcesToClasspath() =
  if (inDevMode) {
    (Compile / resources) ++= {
      ...
      val jsDependencies =
        new File(js.data.getParentFile.absolutePath + s"/pet-project-chat-frontend-jsdeps.js")
    ...
    (Compile / resources) ++= {
      ...
      val jsDependencies =
        new File(js.data.getParentFile.absolutePath + s"/pet-project-chat-frontend-jsdeps.min.js")
